...
(that is, keeping \signal{RXWORDL} = \signal{RXWORDLL}). Any change in
the received word causes a reversion to the original state
\state{BITINC1}.

Assuming three successful delay increments without changing the
received word, we transition to \state{BITBACK} and create a delay

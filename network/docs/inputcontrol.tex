...
\sigal{EVENTSTART} interfaces are analogous.
...
Packet analysis begins in \state{NONE}, and terminates in the
\state{NEXTPKT} state, the latter repeated in Figure
...
\end{centering}
\caption{Input Control FSM. Note that the termiating \state{NEXTPKT} state is repeated for the purpose of clarity. }

...
Note that \signal{ARM} should be deasserted by the source once \signal{GRANT} has been
asserted to prevent multiple-triggering.

When \signal{GRANT} is asserted, that port is given the exclusive control of
the output to the NIC. The \signal{DEN} and and \signal{DIN[15:0]} are passed through with
one-cycle latency.
